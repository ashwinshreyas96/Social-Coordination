...
# --------------------------------------------------
if len(sys.argv) < 5:
    print("Usage: python3 fast_friends_same.py <model_name> <alice_ideology> <jack_ideology> <run_num>")
    sys.exit(1)
...
def create_survey_filename(model_name, alice_ideology, jack_ideology, run_num):
    os.makedirs("./fast_friends_same", exist_ok=True)
    safe_model = model_name.replace(".", "").replace(":", "")
    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    filename = (
        f"./fast_friends_same/affective_{safe_model}_alice-{alice_ideology}_jack-{jack_ideology}_"
...
interaction_log_filename = (
    f"./fast_friends_same/{safe_model_name}_alice-{alice_ideology}_jack-{jack_ideology}_run-{run_num}_"
