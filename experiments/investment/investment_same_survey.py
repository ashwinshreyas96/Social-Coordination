...
import sys
import json
import os  # needed for survey file handling
...
llm_config = LLMConfig(
    ...
    api_type="ollama",
    stream=False,
    client_host="http://127.0.0.1:11449",
)
...
def create_survey_filename(model_name, alice_ideology, jack_ideology, run_num):
    os.makedirs("./invest_same_survey", exist_ok=True)
    safe_model = model_name.replace(".", "").replace(":", "")
    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    filename = (
        f"./invest_same_survey/affective_{safe_model}_alice-{alice_ideology}_jack-{jack_ideology}_"
...
agent1_persona = {
    ...
    "likes": "crocheting, hiking, reading, journaling",
...
decide how much to send this round. The maximum you can get back on your investment is 3 times your investment.
...
log_filename = (
    f"./invest_same_survey/{safe_model_name}_alice-{alice_ideology}_jack-{jack_ideology}_invgame_run-{run_num}_"
