  ...
  for model in "${models[@]}"; do
    ...
    for alice in "${ideologies[@]}"; do
      for jack in "${ideologies[@]}"; do
        ...
        # Run simulation
        python3 pistacchio_same_survey.py "$model" "$alice" "$jack" "$run" \
          > "logs/${safe_model_name}/run_${run}_${alice}_vs_${jack}.log" 2>&1

        # Move generated files into correct folders
        # Simulation CSV (starts with same_survey/)
        sim_file=$(ls same_survey/${safe_model_name}_alice-${alice}_jack-${jack}_run-${run}_*.csv 2>/dev/null)
        if [[ -n "$sim_file" ]]; then
          mv "$sim_file" "sim_results/${safe_model_name}/"
        fi

        # Survey CSV (starts with affective/)
        survey_file=$(ls same_survey/affective_${safe_model_name}_alice-${alice}_jack-${jack}_run-${run}_*.csv 2>/dev/null)
